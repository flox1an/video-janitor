...
async fn sync_relay(
    ...
    let state = db.get_relay_state(relay_url).await?;

    let count = match state {
        None => {
            info!("First sync for relay {}, starting backfill", relay_url);
...
async fn backfill_relay(
    ...
    loop {
        debug!(
            "Backfill batch for {}: until={}, total={}",
            relay_url,
            until.as_u64(),
            total
        );
...
async fn fetch_new_events(
    ...
    debug!("Fetching new events for {} since {}", relay_url, since);
